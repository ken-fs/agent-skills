...
def calculate_resolution_dimensions(aspect_ratio, resolution='2K'):
    """
    根据比例和分辨率计算具体尺寸

    参数：
    - aspect_ratio: 图片比例（'1:1', '4:3', '2.35:1', '16:9', '9:16' 或 '宽:高' 格式）
    - resolution: 分辨率级别（'2K' 或 '4K'）

    返回：分辨率字符串（如 '2560x1440'）
    """
    # 预定义的比例和对应的分辨率
    aspect_ratio_map = {
        '1:1': {
    ...
    if aspect_ratio in aspect_ratio_map:
        return aspect_ratio_map[aspect_ratio].get(resolution, aspect_ratio_map[aspect_ratio]['2K'])
...
def prompt_user_for_aspect_ratio():
    """
    让用户选择图片比例
    ...
    options = [
        {'number': '1', 'ratio': '1:1', 'description': '正方形 - 适合社交媒体头像、Instagram 方形帖子'},
        {'number': '2', 'ratio': '4:3', 'description': '传统屏幕 - 适合演示文稿、传统显示器'},
        {'number': '3', 'ratio': '2.35:1', 'description': '超宽电影 - 适合电影式横幅、宽屏展示'},
        {'number': '4', 'ratio': '16:9', 'description': '宽屏横向 - 适合现代显示器、YouTube、PPT'},
        {'number': '5', 'ratio': '9:16', 'description': '竖屏 - 适合手机、抖音、Instagram Story'},
        {'number': '6', 'ratio': 'custom', 'description': '自定义比例 - 输入自定义宽高比'}
    ]

    for opt in options:
        print(f"{opt['number']}. {opt['ratio']:8s} - {opt['description']}")
        ...
        for opt in options:
            if choice == opt['number']:
                if opt['ratio'] == 'custom':
                    # 自定义比例
                    while True:
                        custom = input("请输入自定义比例（格式: 宽:高，如 3:2）: ").strip()
                        if ':' in custom:
                            try:
                                parts = custom.split(':')
                                float(parts[0])
...
def generate_illustration(section_title, section_content, style_prompt, output_dir, index, aspect_ratio='16:9', resolution='2K'):
    """
    ...
    - output_dir: 输出目录
    - index: 图片序号
    - aspect_ratio: 图片比例（'1:1', '4:3', '2.35:1', '16:9', '9:16' 或自定义）
    ...
    # 根据比例和分辨率计算具体尺寸
    resolution_dimensions = calculate_resolution_dimensions(aspect_ratio, resolution)

    size_instruction = f"\n\n图片规格:\n- 比例: {aspect_ratio}\n- 分辨率: {resolution_dimensions}\n- 质量: {resolution}"

    full_prompt = f"{style_prompt}\n\n根据以下内容生成配图：\n\n标题：{section_title}\n\n内容：{section_content}{size_instruction}"
...
"""
    ...
    parser.add_argument(
        '--aspect-ratio',
        help='图片比例（1:1, 4:3, 2.35:1, 16:9, 9:16 或自定义如 3:2）'
    )
    parser.add_argument(
        '--style',
    ...
    if args.aspect_ratio:
        # 非交互模式：使用命令行参数
        selected_aspect_ratio = args.aspect_ratio
        print(f"\n📐 使用指定比例: {selected_aspect_ratio}")
    else:
        # 交互模式：提示用户选择
        print("\n📐 选择图片比例...")
        selected_aspect_ratio = prompt_user_for_aspect_ratio()

    # 显示计算出的分辨率
    calculated_resolution = calculate_resolution_dimensions(selected_aspect_ratio, args.resolution)
    print(f"   计算分辨率: {calculated_resolution}")
    ...
    print(f"图片比例: {selected_aspect_ratio}")
    print(f"分辨率: {calculated_resolution} ({args.resolution})")
    print("=" * 60)
    print()
